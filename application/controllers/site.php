...
public function deleteenquiries()
{
    ...
    public function exportschedulecsv()
	{
		$access = array("1");
		$this->checkaccess($access);
		$this->match_model->exportschedulecsv();
        $data['redirect']="site/viewmatch";
        $this->load->view("redirect",$data);
	}
//    STUDENT CSV
...
//        echo $id1;
		...
		function uploadswimmingcsv()
	{
		$access = array("1");
		$this->checkaccess($access);
		$data[ 'page' ] = 'uploadswimmingcsv';
		$data[ 'title' ] = 'Upload Swimming Match';
		$this->load->view( 'template', $data );
	}
    function uploadswimmingcsvsubmit()
	{
		$access = array("1");
$this->checkaccess($access);
		$config['upload_path'] = './uploads/';
		...
		$fullfilepath=$filepath."".$file;
		$file = $this->csvreader->parse_file($fullfilepath);
        $id1=$this->match_model->createbycsvswimming($file);
//        echo $id1;

        if($id1==0)
        $data['alerterror']="New Swimming match could not be Uploaded.";
		else
		$data['alertsuccess']="Swimming match Uploaded Successfully.";
		...
		function uploadmedalstallycsv()
	{
		$access = array("1");
		$this->checkaccess($access);
		$data[ 'page' ] = 'uploadmedalstallycsv';
    ...
    function uploadmedalstallycsvsubmit()
	{
		...
		$fullfilepath=$filepath."".$file;
		$file = $this->csvreader->parse_file($fullfilepath);
        $id1=$this->match_model->createbycsvmedalstally($file);
//        echo $id1;

        if($id1==0)
        $data['alerterror']="New Medals Tally could not be Uploaded.";
		else
		$data['alertsuccess']="Medals Tally Uploaded Successfully.";
