...
class Chintantable {
    ...
    public function query($pageno = 1, $maxlength = 20, $orderby = "", $orderorder = "", $search = "", $elements, $from, $where = " WHERE 1 ", $group = "", $having = "", $order = "", $baseurl = "http://localhost/puneetdemo/index.php/site/index", $options = array()) {
        ...
        $return->totalcalc = "SELECT count(*) as `totalcount` FROM ($selectquery " . $fromquery . $wherequery . $groupquery . $havingquery . ") as `tab1`";
        $return->totalvalues = $this->CI->db->query("SELECT count(*) as `totalcount` FROM ($selectquery " . $fromquery . $wherequery . $groupquery . $havingquery . ") as `tab1`")->row();
